...
async function buildDashboardEmbed(settings, roles, guild, client) {
    ...
    const roleList =
        roles.length > 0
            ...
            : '`No hay roles de aplicaciones configurados`';
    ...
    return new EmbedBuilder()
        ...
        .setDescription(`Administra la configuración de aplicaciones para **${guild.name}**.\nSelecciona una opción debajo para modificar una configuración.`)
        .setColor(getColor('info'))
        .addFields(
            { name: 'Estado de aplicaciones', value: settings.enabled ? 'Activado' : 'Desactivado', inline: true },
            { name: 'Canal de registros', value: logChannel, inline: true },
            { name: '\u200B', value: '\u200B', inline: true },
            { name: 'Roles de administradores', value: managerRoleList, inline: false },
            ...
            {
                name: 'Retención',
                value: `Pendientes: **${settings.pendingApplicationRetentionDays ?? 30} días** · Revisadas: **${settings.reviewedApplicationRetentionDays ?? 14} días**`,
                inline: false,
            },
        )
        .setFooter({ text: 'El panel se cerrará después de 15 minutos de inactividad' })
        .setTimestamp();
}
    ...
    return new StringSelectMenuBuilder()
        ...
        .addOptions(
            new StringSelectMenuOptionBuilder()
                ...
                .setDescription('Configura el canal donde se registrarán nuevas aplicaciones')
                .setValue('log_channel')
                .setEmoji('📢'),

            new StringSelectMenuOptionBuilder()
                .setLabel('Roles de administradores')
                .setDescription('Agrega o elimina roles que pueden administrar aplicaciones')
                .setValue('manager_role')
                .setEmoji('🛡️'),

            new StringSelectMenuOptionBuilder()
                .setLabel('Editar preguntas')
                .setDescription('Personaliza las preguntas del formulario de aplicación')
                .setValue('questions')
                .setEmoji('📝'),

            new StringSelectMenuOptionBuilder()
                .setLabel('Agregar rol de aplicación')
                .setDescription('Agrega un rol al que los usuarios puedan postularse')
                .setValue('role_add')
                .setEmoji('➕'),

            new StringSelectMenuOptionBuilder()
                .setLabel('Eliminar rol de aplicación')
                .setDescription('Elimina un rol de la lista de aplicaciones')
                .setValue('role_remove')
                .setEmoji('➖'),

            new StringSelectMenuOptionBuilder()
                .setLabel('Periodo de retención')
                .setDescription('Configura cuánto tiempo se guardan las aplicaciones pendientes y revisadas')
                .setValue('retention')
                .setEmoji('🗑️'),
        );
}
    ...
    async execute(interaction, config, client, selectedAppName = null) {
        try {
            ...
            const applicationsChannel =
                resolveLogChannel(guildConfig, 'applications') || settings.logChannelId;
            ...
            if (isCompletelyUnconfigured) {
                throw new TitanBotError(
                    ...
                    'El sistema de aplicaciones todavía no ha sido configurado. Ejecuta `/app-admin setup` para crear tu primera aplicación.',
                );
            }
            ...
            if (selectedAppName) {
                const selectedRole = roles.find(
                    r => r.name.toLowerCase() === selectedAppName.toLowerCase()
                );
            ...
            throw new TitanBotError(
                `El panel de aplicaciones falló: ${error.message}`,
                ErrorTypes.UNKNOWN,
...
async function showApplicationSelector(interaction, roles, settings, guildId, client) {
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription('Elige qué aplicación deseas configurar.')
        .setColor(getColor('info'));
    ...
    collector.on('end', (collected, reason) => {
        if (reason === 'time' && collected.size === 0) {
            replyUserError(interaction, {
                ...
                message: 'No se seleccionó ninguna opción. El panel se ha cerrado.',
            }).catch(() => {});
        }
    });
}
...
async function showApplicationDashboard(
    rootInteraction,
    selectedRole,
    settings,
    roles,
    ...
    const appSettings = await getApplicationRoleSettings(
        client,
        guildId,
        selectedRole.roleId
    );
    ...
    const logChannelDisplay = appLogChannelId
        ? `<#${appLogChannelId}>`
        : '`Usando canal global de registros`';
    ...
    const questionsDisplay = questions.length > 0
        ? questions
            .map(
                (q, i) =>
                    `${i + 1}. \`${q.length > 60 ? q.substring(0, 60) + '…' : q}\``
            )
            .join('\n')
        : '`Usando preguntas globales`';
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(`Configuración para **${selectedRole.name}**`)
        .setColor(
            isEnabled
                ? getColor('success')
                : getColor('error')
        ...
        .addFields(
            ...
            {
                name: 'Periodo de retención',
                value:
                    `Pendientes: **${settings.pendingApplicationRetentionDays ?? 30} días** · ` +
                    `Revisadas: **${settings.reviewedApplicationRetentionDays ?? 14 días}**`,
                inline: false
            },
        )
        .setFooter({
            text: 'El panel se cerrará después de 10 minutos de inactividad'
        })
        .setTimestamp();
    
       const configMenu = buildApplicationSelectMenu(
        guildId,
        selectedRole.roleId
    );
...
function setupCollectors(
    ...
    collector.on('collect', async selectInteraction => {
        ...
        try {
            ...
            if (error instanceof TitanBotError) {
                ...
                logger.debug(
                    `Error de validación de configuración de aplicaciones: ${error.message}`
                );

            } else {

                logger.error(
                    'Error inesperado en el panel de aplicaciones:',
                    error
                );
            }
        ...
        if (reason === 'time') {
            ...
            const timeoutEmbed = new EmbedBuilder()
                .setTitle('⏰ Panel cerrado por inactividad')
                .setDescription('Este panel se ha cerrado debido a la falta de actividad. Ejecuta el comando nuevamente para continuar.')
    ...
    if (!selectedRoleId) {
        ...
        globalToggleCollector.on('collect', async toggleInteraction => {
            ...
            const deferred =
                await safeDeferInteraction(toggleInteraction);
            ...
            try {
                ...
                await toggleInteraction.followUp({
                    ...
                    embeds: [
                        successEmbed(
                            ...
                            `El sistema de aplicaciones ahora está **${
                                wasEnabled
                                    ? 'desactivado'
                                    : 'activado'
                            }**.\n\n${
                                wasEnabled
                                    ...
                                    ? 'Los miembros ya no podrán enviar solicitudes de aplicación para roles.'

                                    : 'Los miembros ahora podrán comenzar a enviar solicitudes para roles.'
                            }`,
                        )
                    ],
                ...
                logger.error(
                    'Error al cambiar el estado global de aplicaciones:',
                    error
                );
                ...
                await replyUserError(
                    toggleInteraction,
                    {
                        ...
                        message: 'Ocurrió un error al cambiar el estado de las aplicaciones.',
                    }
                );
            }
        });
        ...
        globalToggleCollector.on('end', async (collected, reason) => {
            ...
            if (reason === 'time') {
                ...
                const timeoutEmbed = new EmbedBuilder()
                    ...
                    .setDescription(
                        'Esta sesión del panel se cerró por inactividad (10 minutos).\n\nPara continuar configurando las aplicaciones, ejecuta el comando nuevamente.'
                    )
        ...
        btnCollector.on('collect', async btnInteraction => {
            ...
            const deleteWarningText =
                new TextDisplayBuilder()
                    ...
                    .setContent(
                        `⚠️ Estás a punto de eliminar permanentemente **${appNameForDelete}**. Todas las aplicaciones guardadas y configuraciones de este rol serán eliminadas y no podrán recuperarse.`
                    );
            ...
            confirmModal
                ...
                logger.error(
                    'Error al mostrar el modal de confirmación de eliminación:',
                    error
                );
                ...
                await replyUserError(
                    ...
                    {
                        type: ErrorTypes.UNKNOWN,
                        message: 'No se pudo mostrar el modal de confirmación. Inténtalo nuevamente.',
                    }
                ).catch(() => {});
            ...
            try {
                ...
                const confirmed =
                    confirmSubmit.fields.getCheckbox(
                        'confirm_delete'
                    );
                ...
                if (!confirmed) {
                    ...
                    await replyUserError(
                        ...
                        {
                            type: ErrorTypes.VALIDATION,
                            message: 'Debes marcar la casilla de confirmación para eliminar la aplicación.',
                        }
                    );
                ...
                logger.error(
                    'Error al confirmar eliminación de aplicación:',
                    error
                );
        ...
        toggleCollector.on('collect', async toggleInteraction => {
            ...
            try {
                ...
                const wasEnabled =
                    roles[roleIndex].enabled !== false;



                roles[roleIndex].enabled =
                    !wasEnabled;
                ...
                await toggleInteraction.followUp({
                    ...
                    embeds: [
                        ...
                        successEmbed(
                            ...
                            `La aplicación **${updatedRole.name}** ahora está **${
                                wasEnabled
                                    ? 'desactivada'
                                    : 'activada'
                            }**.\n\n${
                ...
                logger.error(
                    'Error al cambiar estado de aplicación:',
                    error
                );
...
function buildApplicationSelectMenu(guildId, roleId) {
    ...
    return new StringSelectMenuBuilder()
        ...
        .setPlaceholder(
            'Selecciona una configuración para modificar...'
        )
        ...
        .addOptions(
            ...
            new StringSelectMenuOptionBuilder()
                ...
                .setDescription(
                    'Configura el canal donde se registran las aplicaciones'
                )
            ...
            new StringSelectMenuOptionBuilder()
                ...
                .setLabel('Roles administradores')

                .setDescription(
                    'Agrega o elimina roles que pueden administrar aplicaciones'
                )
            ...
            new StringSelectMenuOptionBuilder()
                ...
                .setDescription(
                    'Personaliza las preguntas mostradas en el formulario de aplicación'
                )
            ...
            new StringSelectMenuOptionBuilder()
                ...
                .setDescription(
                    'Configura cuánto tiempo se conservan las aplicaciones pendientes y revisadas'
                )
...
async function handleLogChannel(
    ...
    const modal =
        new ModalBuilder()
            ...
            .setCustomId(
                `app_cfg_log_channel_modal_${guildId}_${selectedRoleId || 'global'}`
            )

            .setTitle(
                'Configurar canal de registros'
            );
    ...
    const channelLabel =
        new LabelBuilder()
            ...
            .setDescription(
                'Canal donde se enviarán las nuevas aplicaciones'
            )
    ...
    try {
        ...
        const channel =
            selectInteraction.guild.channels.cache.get(
                channelId
            );
        ...
        if (selectedRoleId) {
            ...
            embeds: [
                successEmbed(
                    'Canal de registros actualizado',
                    `Los registros de aplicaciones ahora se enviarán a ${
                        channel ?? `<#${channelId}>`
                    }.\nTambién puedes administrar esto desde \`/logging dashboard\`.`
                )
            ],
        ...
        logger.error(
            'Error en el modal del canal de registros:',
            error
        );
...
async function handleManagerRole(
    ...
    const modal =
        new ModalBuilder()
            ...
            .setCustomId(
                `app_cfg_manager_role_modal_${guildId}`
            )

            .setTitle(
                'Configurar roles administradores'
            );
    ...
    const roleSelect =
        new RoleSelectMenuBuilder()
            ...
            .setPlaceholder(
                'Selecciona roles para dar acceso de administrador...'
            )
    ...
    const roleLabel =
        new LabelBuilder()
            ...
            .setDescription(
                'Los roles seleccionados serán activados o desactivados como roles administradores'
            )
    ...
    try {
        ...
        await modalSubmission.reply({
            ...
            embeds: [
                successEmbed(
                    'Roles administradores actualizados',
                    `Roles administradores actuales: ${finalList}`
                )
            ],
        ...
        logger.error(
            'Error en el modal de roles administradores:',
            error
        );
        ...
        await replyUserError(
            ...
            {
                type: ErrorTypes.UNKNOWN,
                message: 'Ocurrió un error al actualizar los roles administradores.',
            }
        );
    }
}
...
async function handleQuestions(
    ...
    const modal =
        new ModalBuilder()
            ...
            .setTitle(
                'Editar preguntas de aplicación'
            )
...
async function handleRoleAdd(
    ...
    const modal =
        new ModalBuilder()
            ...
            .setCustomId(
                `app_cfg_role_add_modal_${guildId}`
            )

            .setTitle(
                'Agregar rol de aplicación'
            );
    ...
    const roleSelect =
        new RoleSelectMenuBuilder()
            ...
            .setPlaceholder(
                'Selecciona el rol al que los miembros podrán aplicar...'
            )
    ...
    const roleLabel =
        new LabelBuilder()
            ...
            .setDescription(
                'Selecciona el rol de Discord al que los miembros podrán solicitar acceso'
            )
    ...
    const nameInput =
        new TextInputBuilder()
            ...
            .setLabel(
                'Nombre visible (déjalo vacío para usar el nombre del rol)'
            )
    ...
    try {
        ...
        const role =
            selectInteraction.guild.roles.cache.get(
                roleId
            );
        ...
        if (
            ...
            await replyUserError(
                ...
                {
                    type: ErrorTypes.UNKNOWN,
                    message: `${role ?? roleId} ya está configurado como un rol de aplicación.`,
                }
            );
        ...
        await modalSubmission.reply({
            ...
            embeds: [
                ...
                successEmbed(
                    ...
                    `${role ?? roleId} fue agregado como **${customName}**.`
        ...
        logger.error(
            'Error en el modal de agregar rol:',
            error
        );
        ...
        await replyUserError(
            ...
            {
                type: ErrorTypes.UNKNOWN,
                message: 'Ocurrió un error al agregar el rol de aplicación.',
            }
        );
    }
}
...
async function handleRoleRemove(
    ...
    const modal =
        new ModalBuilder()
            ...
            .setCustomId(
                `app_cfg_role_remove_modal_${guildId}`
            )

            .setTitle(
                'Eliminar rol de aplicación'
            );
    ...
    const roleLabel =
        new LabelBuilder()
            ...
            .setDescription(
                'Selecciona el rol que será eliminado de la lista de aplicaciones'
            )
    ...
    try {
        ...
        if (index === -1) {
            ...
            await replyUserError(
                ...
                {
                    type: ErrorTypes.USER_INPUT,
                    message: `<@&${roleId}> no está en la lista de roles de aplicación.`,
                }
            );
        ...
        await modalSubmission.reply({
            ...
            embeds: [
                ...
                successEmbed(
                    ...
                    `<@&${roleId}> ha sido eliminado de la lista de aplicaciones.`
        ...
        logger.error(
            'Error en el modal de eliminar rol:',
            error
        );
...
async function handleRetention(selectInteraction, rootInteraction, settings, roles, guildId, client) {
    const modal = new ModalBuilder()
        ...
        .setTitle('Periodos de retención de aplicaciones');

    const retentionInfo = new TextDisplayBuilder()
        .setContent(
            '**Pendientes** — cuánto tiempo se mantienen las aplicaciones sin respuesta o en proceso antes de eliminarse automáticamente.\n' +
            '**Revisadas** — cuánto tiempo se conservan las aplicaciones aprobadas o rechazadas.\n' +
            '-# Ingresa un número entero entre 1 y 3650 (máximo 10 años).',
        );
    ...
    const pendingDays = parseInt(
        submitted.fields.getTextInputValue('pending_days').trim(),
        10
    );

    const reviewedDays = parseInt(
        submitted.fields.getTextInputValue('reviewed_days').trim(),
        10
    );
    ...
    if (isNaN(pendingDays) || pendingDays < 1 || pendingDays > 3650) {
        await replyUserError(submitted, {
            type: ErrorTypes.VALIDATION,
            message: 'La retención de pendientes debe ser un número entero entre **1** y **3650** días.',
        });
        return;
    }
    ...
    if (isNaN(reviewedDays) || reviewedDays < 1 || reviewedDays > 3650) {
        await replyUserError(submitted, {
            type: ErrorTypes.VALIDATION,
            message: 'La retención de revisadas debe ser un número entero entre **1** y **3650** días.',
        });
        return;
    }
    ...
    await submitted.reply({
        embeds: [
            successEmbed(
                ...
                `Las aplicaciones pendientes se conservarán durante **${pendingDays} días**.\nLas aplicaciones revisadas se conservarán durante **${reviewedDays} días**.`,
            ),
        ],
        flags: MessageFlags.Ephemeral,
    });
...
async function handleDeleteApplication(
    ...
    try {
        ...
        await confirmSubmit.reply({
            ...
            embeds: [
                ...
                successEmbed(
                    ...
                    `La aplicación para <@&${selectedRoleId}> (**${deletedRole.name}**) ha sido eliminada permanentemente.\n\n` +
                    `Eliminadas: **${applicationsToDelete.length}** aplicación${
        ...
        logger.error(
            'Error en handleDeleteApplication:',
            error
        );
        ...
        await replyUserError(
            ...
            {
                type: ErrorTypes.UNKNOWN,
                message: 'Ocurrió un error al eliminar la aplicación. Inténtalo nuevamente.',
            }
        );
    }
}
